...
fill_combo_mode (XfburnDeviceBox *box, XfburnDevice *device)
{
  ...
  if (block_types) {
    gtk_list_store_append (GTK_LIST_STORE (model), &iter);
    gtk_list_store_set (GTK_LIST_STORE (model), &iter, MODE_TEXT_COLUMN, "With Track Gaps / Track-At-Once (TAO)", MODE_VALUE_COLUMN, WRITE_MODE_TAO, -1);
  }
  ...
  if (block_types & BURN_BLOCK_SAO) {
    gtk_list_store_append (GTK_LIST_STORE (model), &iter);
    gtk_list_store_set (GTK_LIST_STORE (model), &iter, MODE_TEXT_COLUMN, "Multi Session / Session-At-Once (SAO)", MODE_VALUE_COLUMN, WRITE_MODE_SAO, -1);
  }
  /*
